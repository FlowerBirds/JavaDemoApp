...
public class FeiqUtil {
    ...
    public static GroupMsg getGroupMsg(byte[] data) throws Exception {
    ...
    private static boolean isGroupMsgType(byte[] data) throws Exception {
        byte[] typeByte = getIndexValue(data, 4, ':', 0);
    ...
    private static String getPcName(byte[] data) throws Exception {
        return new String(getIndexValue(data, 3, ':', 0), DEFAULT_ENCODING);
    }

    private static byte[] getIndexValue(byte[] data, int startIndex, char charValue, int length) {
        int count = 0, p = 0;
        ...
        for (byte b : data) {
            ...
            p++;
            if (length <= 0 && count == startIndex + 1) {
                break;
            }
            if (findFlag) {
                byteList.add(b);
            }
            if(length > 0 && byteList.size() == length) {
            	break;
            }
            if (count == startIndex) {
                findFlag = true;
    ...
    public static byte[] getSendData(long groupNum, String msg, String userName, String pcName, String mac) throws Exception {
        ...
        byte[] headData = String.format("1_lbt80_0#128#%s#0#0#%d#4000#9:%d:%s:%s:%d:", mac, groupEncryptData.length, (System.currentTimeMillis() / 1000), pcName, userName, MSG_TYPE_GROUP).getBytes(DEFAULT_ENCODING);
    ...
    //Blowfish加密
    public static byte[] encrypt(String keyValue, byte[] encryptData) throws Exception {
    ...
    //Blowfish解密
    public static byte[] decrypt(String keyValue, byte[] decryptData) throws Exception {
        SecretKeySpec key = new SecretKeySpec(keyValue.getBytes(), "Blowfish");
        IvParameterSpec iv = new IvParameterSpec(new byte[]{0, 0, 0, 0, 0, 0, 0, 0});
        Cipher cipher = Cipher.getInstance("Blowfish/CBC/PKCS5Padding");
